...
.DESCRIPTION
    ...
    → 服务器端 docker build → 重建容器 → HTTPS 冒烟检查。
...
try {
    ...
    # ---------- 5. 服务器端构建 + 重建容器（服务会中断十几秒） ----------
    Step "服务器端 docker build 并重建容器"
    $remote = "set -e; cd $RemoteDir; rm -rf app web/dist; tar -xzf /tmp/watchtower_deploy.tar.gz; rm /tmp/watchtower_deploy.tar.gz; docker build -t $Image .; docker rm -f $Container >/dev/null 2>&1 || true; docker run -d --name $Container --restart unless-stopped -p 127.0.0.1:8788:8788 -v $RemoteDir/data:/data --env-file $RemoteDir/watchtower.env $Image"
    ssh $Server $remote
