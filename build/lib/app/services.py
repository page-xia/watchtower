...
class DashboardService:
    ...
    def signal_detail_overlay(
        ...
        opening_markers = self._opening7_overlay_markers(info, bundle.rows, transaction_flow)
    ...
    def signal_detail_f10(self, code: str) -> Any:
        """个股详情页 F10 聚合数据（tushare_pro + easy_tdx），按需加载，有 TTL 缓存。"""
        return self.data_source.fetch_f10_full(code)
    ...
    def _zuot_formula_overlay(formula_result: Any, prev_close: float) -> FormulaOverlay:
        ...
        if not states or day is None or base <= 0 or not day.levels_available:
        ...
        return FormulaOverlay(
            available=True,
            ma30_pcts=[pct(state.get("ma30")) for state in states],
            qs_pcts=[pct(state.get("qs")) for state in states],
